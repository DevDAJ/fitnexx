import type { ChartAnimationOptions, ChartCursorPresentation, DomChartDefinition, ChartFocusState, ChartHostControl, ChartHostControlExtensionToken, ChartMotionTransition, ChartPoint, ChartScene, ChartSvgRenderer, ChartTextMeasurer, ChartTooltipBodyContext, ChartTooltipExtensionToken, ChartTooltipOptions, ChartTooltipPlacement, ChartTooltipPortalExtensionToken, ChartTooltipPortalOptions, ChartTooltipPosition, ChartValue, RenderChartOptions, RenderChartSvgOptions } from './types.js';
...
export interface ChartTooltipMotionController {
    ...
    afterPaint: (element: HTMLElement, snapshot: ChartTooltipMotionSnapshot, transition: false | ChartMotionTransition | undefined) => void;
    hide: (element: HTMLElement, transition: false | ChartMotionTransition | undefined, complete: () => void) => boolean;
...
export interface UniversalChartRenderer {
    readonly id: string;
    readonly capabilities?: ChartRendererCapabilities;
    prerender: <TDatum, TXValue extends ChartValue = ChartValue, TYValue extends ChartValue = ChartValue>(scene: ChartScene<TDatum, TXValue, TYValue>, options: RenderChartOptions) => string;
    mount: <TDatum, TXValue extends ChartValue = ChartValue, TYValue extends ChartValue = ChartValue>(container: HTMLElement, requestRender: (force?: boolean) => void) => ChartSurface<TDatum, TXValue, TYValue>;
}
