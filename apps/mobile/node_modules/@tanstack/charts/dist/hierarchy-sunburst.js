...
function sunburst(source, options) {
  ...
  if (options.visibleDepth !== void 0) {
  ...
  const nodes = partitioned.descendants().slice(1).filter((node) => node.depth <= visibleDepth && node.x1 > node.x0).map((node) => ({
    node: context(node),
    start: node.x0,
    end: node.x1
  }));
...
function projectSunburstSectorPath(values) {
  const [startAngle, endAngle, innerRadius, outerRadius] = values;
  if (startAngle === void 0 || endAngle === void 0 || innerRadius === void 0 || outerRadius === void 0) {
    return void 0;
  }
...
function resolveLayoutRoot(root, rootId) {
  if (rootId === void 0) return root.copy();
  if (typeof rootId !== "string" || rootId.length === 0) {
    throw new TypeError("sunburst: rootId must be a nonempty string");
  }
  const selected = root.descendants().find((node) => node.data.id === rootId);
  if (!selected) {
