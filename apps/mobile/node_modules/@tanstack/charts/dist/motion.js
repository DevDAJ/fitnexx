...
function createMotionSvgChartRenderer(motion2, renderSvg = renderChartSvgWithResources, ownerRenderer) {
  const renderer = {
    id: `svg:${motion2.id}`,
    capabilities: {
      tooltipMotion: {
        protocol: 1,
        createController: motion2.createTooltip
      }
    },
    prerender: renderSvg,
    mount(container) {
...
function addSemanticPathUpdateTrack(current, next, tracks, context, timingContext) {
  ...
  const resolvedContext = timingContext ?? elementTimingContext(current, "update", context.scene);
...
function hierarchyRelatedGeometry(element, ownerScene, relatedScene, phase) {
  const relation = hierarchyMotionRelation(element, ownerScene, relatedScene);
  if (!relation) return void 0;
  const source = phase === "enter" ? relation.related.metadata.path : relation.owner.metadata.path;
  const target = phase === "enter" ? relation.owner.metadata.path : relation.related.metadata.path;
...
function hierarchyMotionRelation(element, ownerScene, relatedScene) {
  ...
  for (let index = hierarchy.ancestorIds.length - 1; index >= 0; index -= 1) {
    ...
    ancestor = related.find(
      (entry) => entry.metadata.hierarchy?.markId === hierarchy.markId && entry.metadata.hierarchy.id === ancestorId
    );
    if (ancestor) break;
  }
  if (!ancestor) return void 0;
  const root = element.closest("svg");
  const relatedElement = root ? [...root.querySelectorAll("path[data-ts-key]")].find(
    (candidate) => candidate.getAttribute("data-ts-key") === ancestor.node.key
  ) : void 0;
  const relatedPath = relatedElement?.getAttribute("d");
  if (!relatedElement || !relatedPath) return void 0;
  return {
    owner,
    related: ancestor,
    relatedElement,
...
const sceneMotionEntriesCache = /* @__PURE__ */ new WeakMap();
function sceneMotionEntries(scene) {
  const cached = sceneMotionEntriesCache.get(scene);
  if (cached) return cached;
  const entries = [];
  const visit = (nodes) => {
    for (const node of nodes) {
      const metadata = node[sceneMotionNode];
...
function sceneMotionEntry(scene, key) {
  return sceneMotionEntries(scene).find((entry) => entry.node.key === key);
}
function compatiblePathGeometry(source, target) {
  if (!source || !target || source.project !== target.project || source.values.length !== target.values.length || source.values.length === 0) {
    return void 0;
  }
  return { source, target };
}
function livePathGeometryValues(element, geometry, runtime) {
  const states = elementValueStates(
    runtime,
    element,
    "semantic-path",
  ...
  return staticPath && element.getAttribute("d") === staticPath ? geometry.values : states.map((state) => state.value);
}
...
function resolveTiming(options, context, definitions, overrides) {
  ...
  const apply = (definition) => {
    ...
    disabled = false;
    const authoredDelay = typeof authored.delay === "function" ? authored.delay(context) : authored.delay;
    if (authoredDelay !== void 0) {
