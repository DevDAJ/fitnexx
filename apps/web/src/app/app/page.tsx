import { Activity, Dumbbell, Settings, TrendingUp, Utensils } from "lucide-react";
...
export default function AppHome() {
  return (
    ...
    >
      ...
      >
        <Text fontSize={16} fontWeight="800" color="$color" letterSpacing={-0.02}>
          Fitnexx
        </Text>
        <View
          width={32}
      ...
      <View flex={1} paddingVertical={24} paddingHorizontal={16}>
        <YStack maxWidth={1024} width="100%" marginLeft="auto" marginRight="auto" gap={24}>
          <YStack gap={4}>
          ...
          <XStack flexWrap="wrap" gap={12}>
            <KpiCard title="PRs" value="12" subtitle="last 30 days" delta="up" color="#fbbf24" />
            <KpiCard title="Volume" value="48.2k" subtitle="last 30 days" delta="up" color="#3b82f6" />
            <KpiCard title="Weekly Sets" value="9.4" subtitle="avg / muscle" delta="same" color="#8b5cf6" />
          </XStack>
          ...
          <Card gap={12} flexDirection="row" alignItems="center">
...
function KpiCard({
  ...
  return (
    <Card
      flexBasis="100%"
      $sm={{ flexBasis: "calc(33.333% - 8px)" as any }}
      flexGrow={1}
      gap={4}
      backgroundColor="$card"
    >
      <Text color="$muted" fontSize={12} fontWeight="600" textTransform="uppercase" letterSpacing={0.5}>
        {title}
      </Text>
...
function QuickAction({ icon: Icon, label }: { icon: React.ComponentType<{ size?: number; color?: string; style?: React.CSSProperties }>; label: string }) {
  return (
    <Card
      flexDirection="row"
      alignItems="center"
      gap={10}
      flexBasis="100%"
      $sm={{ flexBasis: "calc(50% - 6px)" as any }}
      flexGrow={1}
    >
      <View
        width={36}
